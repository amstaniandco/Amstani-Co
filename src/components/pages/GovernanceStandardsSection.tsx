...
export default function GovernanceStandardsSection() {
  return (
    <section className="relative w-full overflow-hidden bg-[#f6f7f9] py-12 sm:py-14">
      <div className="relative w-full px-5 sm:px-8 lg:px-12">
        <h2 className="text-left text-[30px] font-semibold tracking-tight text-[#0f172a] sm:text-[42px]">
          ...
          {governanceCards.map((card) => {
            ...
            return (
              <article
                ...
                className="min-h-[320px] rounded-[26px] border border-[#d9e6ec] bg-white px-6 py-8 shadow-[0_14px_35px_rgba(15,23,42,0.08)] sm:min-h-[350px] sm:px-8 sm:py-9"
              >
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[#e6f4f7] ring-1 ring-[#c3e5ec]">
                  <Icon className="h-8 w-8 text-[#2f9fb3]" />
                </div>

                <h3 className="mt-5 text-center text-[24px] font-semibold leading-tight text-[#1e293b] sm:text-[30px]">
                  {card.title}
                </h3>

                <p className="mt-4 text-center text-base leading-relaxed text-[#64748b] sm:text-[18px]">
                  {card.description}
                </p>
              </article>
            );
