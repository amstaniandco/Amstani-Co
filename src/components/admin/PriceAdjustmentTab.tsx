...
export default function PriceAdjustmentTab() {
  ...
  async function apply() {
    ...
    if (isNaN(pct) || pct <= 0) { setMsg({ ok: false, text: "Enter a percentage greater than 0." }); return; }
    if (mode === "decrease" && pct >= 100) { setMsg({ ok: false, text: "Decrease must be less than 100%." }); return; }
    ...
    try {
      ...
      if (!res.ok) { setMsg({ ok: false, text: data.error || "Failed to apply." }); return; }
      setMsg({ ok: true, text: `${mode === "increase" ? "Increased" : "Decreased"} ${data.updatedProducts} product prices by ${pct}%.` });
      setPctInput("");
  ...
  async function reset() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setMsg({ ok: false, text: data.error || "Failed to reset." }); return; }
      setMsg({ ok: true, text: `Reset ${data.updatedProducts} products to their original prices.` });
      setPctInput("");
    } finally { setResetting(false); }
  }

  const preview = pctInput && parseFloat(pctInput) > 0
    ? mode === "increase"
  ...
  return (
    <div className="mx-auto max-w-2xl">
      {/* Header */}
      <div className="mb-5">
        <p className="text-xs text-slate-400">Apply a single percentage to every product&apos;s base price across the catalog.</p>
        <h2 className="mt-1 text-2xl font-extrabold tracking-tight text-slate-900">Price Adjustment</h2>
      </div>

      {/* Feedback */}
      {msg && (
        <div className={`mb-4 flex items-start justify-between rounded-xl px-4 py-3 text-sm ${msg.ok ? "bg-green-50 text-green-800 border border-green-200" : "bg-red-50 text-red-800 border border-red-200"}`}>
          <span>{msg.text}</span>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="text-xs text-slate-500 mb-5">
          Increase or decrease <span className="font-semibold">all product prices</span> by a percentage.
          The original price is always preserved — use <span className="font-semibold">Reset</span> to restore it.
        </p>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          >
            {applying ? "Applying…" : `Apply ${mode === "increase" ? "Increase" : "Decrease"}`}
          </button>
        ...
        {preview && (
          <p className="mt-4 text-xs text-slate-500">
            Preview: a <span className="font-semibold">$100.00</span> product becomes{" "}
            <span className={`font-bold ${mode === "increase" ? "text-[#2f7f8d]" : "text-amber-600"}`}>${preview}</span>
          </p>
        )}
      </div>
    </div>
