...
export async function GET(req: Request) {
  try {
    ...
    const categories = await client.db(DB_NAME).collection("categories").find(query).sort({ name: 1 }).limit(500).toArray();

    return NextResponse.json({ categories }, { status: 200 });
  } catch (error) {
    console.error("GET /api/admin/global-catalog/categories error:", error);
