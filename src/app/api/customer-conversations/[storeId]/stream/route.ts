...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if (user.role === "owner") {
    ...
    if (!chatId || !ObjectId.isValid(chatId)) return new Response("chatId required", { status: 400 });
  } else {
        ...
        try {
          if (chatId && ObjectId.isValid(chatId)) {
            const messages = await db
              ...
              .find({ chatId: new ObjectId(chatId), _id: { $gt: lastId } })
              .sort({ _id: 1 })
              .toArray();
            ...
            const updatedMessages = await db
              .collection("customer_messages")
              .find({ chatId: new ObjectId(chatId), updatedAt: { $gt: lastUpdateCheck } })
              .sort({ updatedAt: 1 })
              .toArray();
